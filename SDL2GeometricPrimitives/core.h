...
{
public:
	Triangle(SDL_FPoint x, SDL_FPoint y, SDL_FPoint z) : IPrimitive(), verts{
		SDL_Vertex{ x, SDL_Color{ 255, 0, 0, 255 }, SDL_FPoint{ 0 } },
		SDL_Vertex{ y, SDL_Color{ 0, 0, 255, 255 }, SDL_FPoint{ 0 } },
		SDL_Vertex{ z, SDL_Color{ 0, 255, 0, 255 }, SDL_FPoint{ 0 } } }
	{
		
	}
	Triangle(SDL_FPoint x, SDL_FPoint y, SDL_FPoint z, SDL_Color color) : IPrimitive(), verts{
		SDL_Vertex{ x, SDL_Color{ color }, SDL_FPoint{ 0 } },
		SDL_Vertex{ y, SDL_Color{ color }, SDL_FPoint{ 0 } },
		SDL_Vertex{ z, SDL_Color{ color }, SDL_FPoint{ 0 } } } 
	{
	
	}
	Triangle(SDL_FPoint x, SDL_FPoint y, SDL_FPoint z, SDL_Color xc, SDL_Color yc, SDL_Color zc) : IPrimitive(), verts{
		SDL_Vertex{ x, SDL_Color{ xc }, SDL_FPoint{ 0 } }, 
...
class Rectangle final : public IPrimitive
{
public:
	Rectangle() = delete;
...
class Circle final : public IPrimitive
{
	struct Point
	{
		int x;
...
public:
	SDLBox()
	{
		...
		surfaceMessage = std::unique_ptr<SDL_Surface, std::function<void(SDL_Surface*)>>(
			TTF_RenderText_Solid(font, "press ESC to quit", { 255, 255, 255 }),
			[](SDL_Surface* surfaceMessage) { SDL_FreeSurface(surfaceMessage); });
		assert(surfaceMessage);
		
		message = std::unique_ptr<SDL_Texture, std::function<void(SDL_Texture*)>>(
